...
ctx.fillText("", (canvas.width / 2) - 17, (canvas.height / 2) + 8);
let cw = (canvas.width = window.innerWidth);
let ch = canvas.height/*(canvas.height = window.innerHeight)*/;
...
class Particle {
  constructor() {
    ...
    this.color = Math.random() < .1 ? "hsla(0,90%,50%,1)" : "hsla(280,80%,50%,.7)";
    this.history = [];
  }
...
function Init() {
  ...
  ch = canvas.height = window.innerHeight; //300;
